...
class ThreadTool {
 public:
  static void SetNameForCurrentThread(std::string_view thread_name) {
  ...
  static void SetCpuSchedForCurrentThread(std::string_view sched) {
    ...
    if (sched == "SCHED_OTHER") {
      ...
      AIMRT_ASSERT(sched_priority >= priority_min && sched_priority <= priority_max,
                   "Invalid sched priority '{}', required range {}~{}", sched_priority,
 ...
 private:
  ThreadTool() = default;
  ~ThreadTool() = default;
  ThreadTool(const ThreadTool&) = delete;
  ThreadTool& operator=(const ThreadTool&) = delete;
};
